...
class Motor():
    ...
    def set_power(self, onoff=False):
        en = "/dev/rtmotoren0"
        try:
            with open(en, 'w') as file:
                file.write("1\n" if onoff else "0\n")
            self.is_on = onoff
            return True
        except:
            rospy.logerr("cannot write to {}".format(en))
            return False
    ...
    def set_raw_freq(self, left_hz, right_hz):
        ...
        try:
            with open("/dev/rtmotor_raw_l0", 'w') as Lfile, open("/dev/rtmotor_raw_r0", 'w') as Rfile:
                Lfile.write(str(int(round(left_hz))) + '\n')
                Rfile.write(str(int(round(right_hz))) + '\n')
        except:
    ...
    def callback_tm(self, message):
        if not self.is_on:
            rospy.logerr("not yet enpowered")
            return False

        dev = "/dev/rtmotor0"
        try:
            with open(dev, 'w') as file:
                file.write("{:d} {:d} {:d}\n".format(message.left_hz, message.right_hz, message.duration_ms))
            return True
        except:
            rospy.logerr("cannot write to {}".format(dev))
            return False
